...
export default function MultiActionAreaCard() {
  ...
  useEffect((image) => {
    const loadImage = async () => {
      const res = await fetch(`http://localhost:5000/images/${globalStore.user.avatar}`);
      const imageBlob = await res.blob();
      const imageObjectURL = URL.createObjectURL(imageBlob);
      console.log(res.url);
      setImage(res.url);
      setLoading(false);
      return new File([imageBlob], globalStore.user.avatar, {type: imageBlob.type})
    }
    const presetValue = () => {
      reset({
  ...
  const onSubmit = async (data,event) => {
    ...
    if(data.avatar[0]) {
      ...
      const response = await fetch(`http://localhost:5000/images/${globalStore.user.avatar}`);
    ...
    await axios.put(`http://localhost:5000/api/users/${globalStore.user.id}`, formData, {
      headers: {
