...
export default function MultiActionAreaCard() {
    ...
    const fetchData = async () => {
      if (fetching && globalStore.basketId) {
        const response = await fetch(
          `http://localhost:5000/api/users/${globalStore.user.id}/baskets/${globalStore.basketId}/books`,
          {
            method: "GET",
            headers: {
  ...
  const deleteBook = async (bookId) => {
    const response = await fetch(
      `http://localhost:5000/api/users/${globalStore.user.id}/baskets/${globalStore.basketId}/books/${bookId}`,
      {
        method: "DELETE",
      }
    );
  ...
  const createOrder = async () => {
    const response = await fetch(
      `http://localhost:5000/api/users/${globalStore.user.id}/orders`,
      {
        method: "POST",
        body: JSON.stringify({
